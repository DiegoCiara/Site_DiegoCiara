export const Profile ={
  image:'/Profile/profile.JPG',
  logo: null,
  name:'Diego Ciara',
  age:'23',
  profession:'Programador JavaScript',
  phone:'8197052688', 
  mail: 'diegociara.dev@gmail.com',
  github:'https://github.com/DiegoCiara',
  linkedin:'https://www.linkedin.com/in/diegociara',
  instagram:'https://www.instagram.com/sr.ciara',
  resume:'Olá! Me chamo Diego, tenho 22 anos, sou programador front-end e acadêmico em Engenharia de Software com o maior amor e orgulho!',
}


export const TopProjects = [
  {
    type:'Website',
    name:'União Promotora',
    image:'CardsProjects/uniao.svg',
    description:'Site comercial para Promotora de crédito com simulações de análise de crédito, integrado com sistema de login de usuários',
    path: null,
    deploy: 'https://uniaoconsignado.com.br',
  },
  {
    type:'Website',
    name:'Figio',
    image:'CardsProjects/figio.svg',
    description:'Site comercial para agência de marketing integrado com sistema de pagamentos cadastro de clientes de login de usuários',
    path: null,
    deploy:  'https://figio.com.br',
  },
  
  {
    type:'Website',
    name:'Softspace',
    image:'CardsProjects/softspace.svg',
    description:'Site comercial de plataforma de gestão para empresas de tecnologia e negócios de recorrência, com ênfase em UI/UX e notificações de e-mails.',
    path: null,
    deploy:  'https://softspace.com.br',
  },
  {
    type:'SaaS',
    name:'Wave CRM',
    image:'CardsProjects/wave.svg',
    description:'CRM de vendas para empresas B2B, com multiplos funís de vendas, cadastro de contatos, logins de usuários, automações de fluxos e mais.',
    path: 'https://github.com/DiegoCiara/Wave_CRM',
    deploy:'https://wave.wavecrm.com.br/'
  },
  {
    type:'Tool',
    name:'NewsLetter',
    image:'CardsProjects/newsletter.svg',
    description:'Captação de leads para cadastro de portal de notícias via e-mail marketing',
    path: 'https://github.com/DiegoCiara/Newsletter',
    deploy:'https://newsletter-blue.vercel.app/'
  },
  {
    type:'Tool',
    name:'Guide',
    image:'CardsProjects/guide.svg',
    description:'Construtor de guia de software, manual de ferramenta e documentação de API.',
    path: 'https://github.com/DiegoCiara/Guide',
    deploy:'https://guide-two.vercel.app/',
  },
  {
    type:'Mobile',
    name:'Sinergy',
    image:'CardsProjects/sinergy.svg',
    description:'Aplicativo de rede social para empresas, voltado ao aumento do engajamento interno de colaboradores',
    path: 'https://github.com/DiegoCiara/Sinergy_Mobile',
    deploy: null,
  },
  {
    type:'IA',
    name:'Private GPT4',
    image:'CardsProjects/GPT-4.jpg',
    description:'Inteligência Artificial para processamento de linguagem utilizando o GPT-4',
    path: 'https://github.com/DiegoCiara/privateGPT4',
    deploy: null,
  },
  
]


export const Projects = [
  {
    type:'SaaS',
    name:'Wave CRM',
    image:'https://www.wavecrm.com.br/Home/mockup.svg',
    description:'CRM de vendas para empresas B2B',
    path: 'https://github.com/DiegoCiara/Wave_CRM'
  },
  {
    type:'Tool',
    name:'NewsLetter',
    image:'https://www.wavecrm.com.br/Home/mockup.svg',
    description:'Aplicativo de rede social para empresas',
    path: 'https://github.com/DiegoCiara/Newsletter',
    deploy:'https://newsletter-blue.vercel.app/'
  },
  {
    type:'Site',
    name:'Portifólio Dev',
    image:'https://www.wavecrm.com.br/Home/mockup.svg',
    description:'Site portifólio para desenvolvedores',
    path: 'https://github.com/DiegoCiara/Portifólio',
    deploy:'https://diegociara.com.br'
  },
  {
    type:'LandingPage',
    name:'Sound App',
    image:'https://github.com/DiegoCiara/Sound-App/raw/master/public/landing-page.png',
    description:'Landing Page simples para plataforma de streaming',
    path: 'https://github.com/DiegoCiara/Sound-App'
  },
  {
    type:'Tool',
    name:'SendMail Resend',
    image:'https://github.com/DiegoCiara/Sound-App/raw/master/public/landing-page.png',
    description:'Envio de e-mails utilizando Resend',
    path: 'https://github.com/DiegoCiara/Sendmail_Resend'
  },
  {
    type:'Tool',
    name:'Guide',
    image:'/src/assets/CardsProjects/Guide.png',
    description:'Construtor de Documentação',
    path: 'https://github.com/DiegoCiara/Guide',
    deploy:'https://guide-two.vercel.app/',
  },
  {
    type:'Mobile',
    name:'Sinergy',
    image:'/src/assets/CardsProjects/Sinergy.png',
    description:'Aplicativo de rede social para empresas',
    path: 'https://github.com/DiegoCiara/Sinergy_Mobile'
  },
  {
    type:'Mobile',
    name:'AppBank',
    image:'/src/assets/CardsProjects/App-bank.png',
    description:'Aplicativo de controle financeiro pessoal',
    path: 'https://github.com/DiegoCiara/App-bank'
  },
  {
    type:'IA',
    name:'private GPT4',
    image:'/src/assets/CardsProjects/GPT-4.jpg',
    description:'Inteligência Artificial para processamento de linguagem utilizando o GPT-4',
    path: 'https://github.com/DiegoCiara/privateGPT4'
  },
]
